<aside>
  ...
  <?php elseif( ICL_LANGUAGE_CODE == 'en' ): //英語 ?>
    <div class="side-login">
      <p class="ttl">Log In</p>
      <?php if ( is_user_logged_in() ) : ?>
        <?php
        $current_user = wp_get_current_user();
        ?>
        <div class="now-login">
          <p class="logged-in-message"><?php echo 'Hello : ' . esc_html( $current_user->display_name ); ?></p>
          <a href="<?php echo wp_logout_url( get_permalink() ); ?>">Log Out</a>
        </div>
      <?php else : ?>
        <form name="loginform" id="loginform" action="<?php echo wp_login_url(); ?>" method="post">
          <p class="id">
            <label for="user_login">LogIn ID<br />
            <input type="text" name="log" id="user_login" class="input" value="" size="20" /></label>
          </p>
          <p class="pass">
          ...
          <p class="submit">
            <input type="submit" name="wp-submit" id="wp-submit" class="btn01 button button-primary" value="LogIn" />
            <input type="hidden" name="redirect_to" value="<?php echo home_url('/my-account'); ?>" />
          </p>
          <p>
            <a href="<?php echo home_url('/'); ?>my-account/lost-password/">Forgot your password?</a>
          </p>
        </form>
      <?php endif; ?>
    </div><!--side-login-->
  ...
  <?php elseif( ICL_LANGUAGE_CODE == 'en' ): //英語 ?>
    <div class="side-cart">
      ...
      if ( $cart->get_cart_contents_count() > 0 ) {
        ...
        echo '<p class="no-cartitem">There are no products in your cart.</p>';
      }
      ?>
    </div><!-- /.side-cart -->
  <?php endif; ?>
    ...
    <div class="side-cate">
      ...
      <nav>
        <ul>
          ...
          <li><a href="<?php echo home_url('/'); ?>product-category/fresh-noodles-en/">Fresh-Noodles</a></li>
          <li><a href="<?php echo home_url('/'); ?>product-category/dried-noodles-en/">Dried-Noodles</a></li>
          <li><a href="<?php echo home_url('/'); ?>product-category/ll-noodles-en/">LL-Noodles</a></li>
          <li><a href="<?php echo home_url('/'); ?>product-category/broth-en/">Broth</a></li>
        </ul>
      </nav>
    </div><!-- /.side-cate -->

    <div class="btn-wrap"><a class="btn02" href="<?php echo home_url('/'); ?>faq/">FAQ</a></div>
